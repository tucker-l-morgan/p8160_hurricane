...
# this will calculate the mean estimate for each coefficient
beta_vec[2551:5000,] %>% # we want to specify the second half of runs 
  group_by(beta) %>% 
...
ggplot(data = beta_vec, aes(y = estimate, col = beta)) +
  geom_line(aes(ind)) +
  coord_cartesian(xlim = c(length(beta_res_list) - 100, length(beta_res_list)))
...
mean(sigma2_vec[500:1000])

Reduce("+", test$sigma_m[500:1000]) / length(test$sigma_m[500:1000])
