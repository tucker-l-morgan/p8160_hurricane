...
theta <- list(
  "beta" = list("beta" = matrix(data = 0, nrow = 1, ncol = 5),
                "mu" = 0,
                "sigma" = 0),
  "B" = list(matrix(data = 0, nrow = length(train_list$x), ncol = 5),
             "mu" = 0,
             "sigma" = 0),
  "sigma2" = c(1),
...
train_list <- list(y = list(), x = list())
test_list <- list(y = list(), x = list())
...
for  (i in 1:nrow(hurr_df)) {
  ...
  train.y <- y[1:ntrain]
  train.x <- x[1:ntrain,]
  train_list$y[[i]] <- train.y
  train_list$x[[i]] <- train.x
  
  test.y <- y[(ntrain + 1):m]
  test.x <- x[(ntrain + 1):m,]
  test_list$y[[i]] <- test.y
  test_list$x[[i]] <- test.x
}
```
...
beta_dist <- function(sigma, B, n){
  # sigma stored as inverse
  u_matrix <- matrix(0, nrow = 5, ncol = n)
  for(i in 1:n){
  ...
  mu = (solve(sigma, tol = 1e-95) / n) %*% matrix(u, nrow = 5)
  sigma = psych::cor.smooth(solve(sigma, tol = 1e-95) / n)
...
sigma_m_dist <- function(beta, B, n){
...
gibbs_fun <- function(iter, start, data){
  ...
  for (i in 2:iter){
    #browser()
    beta_list[[i]] <- beta_dist(sigma = sigma_list[[i-1]], B = B_list[[i-1]][[1]], n = n)
    sigma_list[[i]] <- sigma_m_dist(beta = matrix(beta_list[[i]][[1]], ncol = 5), B = B_list[[i-1]][[1]], n = n)
...
train_res <- gibbs_fun(iter = 10, start = theta, data = train_list)
```
