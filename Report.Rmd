...
To obtain estimates of $\Theta = (\textbf{B}^{T}, \boldsymbol{\beta}^{T}, \sigma^2, \Sigma)$, we first generated 1000 iterations in the Markov Chain. As an illustration, **Figure 1** shows the Markov Chain for $\boldsymbol{\beta}$ estimates. Although there is some noise in the chart, we can see that $\beta_1$ and $\beta_4$, the coefficients associated with wind speed at time $t$ and the change in wind speed from $t-6$ to $t$, tend to fluctuate around $1$, whereas the remaining coefficients fluctuate around $0$.

```{r MCMC beta viz, echo = FALSE}
...
ggplot(data = beta_vec, aes(x = estimate, fill = beta)) +
  ...
  geom_vline(data = beta_estimate, mapping = aes(xintercept = avg_est), lty = "dashed") +
  ...
  labs(x = "Beta Estimates Across Hurricanes",
       ...
       caption = "Note: Vertical lines indicate estimates of average beta values.")
```
