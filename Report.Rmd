...
ggplot(data = B_vec, aes(x = estimate, fill = beta)) +
  ...
  labs(x = "Beta Estimates Across Hurricanes",
       ...
       caption = "Note: Vertical lines indicate average estimates of beta values across hurricanes.") +
...
```{r plotting mse}
mse_vec <- data.frame(do.call(rbind, mse)) %>% 
  rename(mse = 1) %>% 
